...
Run:
    ...
    For proxy on port "8080" with server "https://httpbin.org/" (https://localhost:8080) (use ssl)
...
Tests:
    ...
    com.nyshup.ReverseProxyTest: Integration test. Should be run when server is runned on port 8080 with ssl.
...
Dynamic:
    ...
    After receiving HttpRequest part of request we could analyze request and change behaviour.
    Next functionality is implemented:
        - Server could replace next parameters if some headers will be found:
            Header "X-other-remote-ssl" replace parameter passed in --rport 
            Header "X-other-remote-ssl" replace parameter passed in --rhost  
            Header "X-other-remote-ssl" replace parameter passed in --rssl
        - If url finished with "traffic" statistic information is returned
            To check: https://127.0.0.1/traffic
...
To secure proxy RuleBasedIpFilter is used.
    ...
    Example:
        java -jar ./target/reverse-proxy-1.0-SNAPSHOT.jar --port 8080 --rhost httpbin.org --rport 443 --ipFilter 127.0.0.1

    
Bonus:

    java -jar ./target/reverse-proxy-1.0-SNAPSHOT.jar --port 8080 --rhost www.meldium.com --rport 443 --ssl false